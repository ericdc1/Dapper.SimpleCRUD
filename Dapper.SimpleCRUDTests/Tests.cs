...
namespace Dapper.SimpleCRUDTests
{
    ...
    [MetadataType(typeof(AutogeneratedClassMetadata))]
    public class AutogeneratedClass
    {
        internal sealed class AutogeneratedClassMetadata
        {
            [Required]
    ...
    public class Tests
    {
        ...
        public void TestInsertingObjectWithExtraMetadata()
        {
            using (var connection = GetOpenConnection())
            {
                var autoGenerated = new AutogeneratedClass { Id = 5, Name = "Bob" };
                connection.Insert(autoGenerated);
                var result = connection.Query<AutogeneratedClass>("select * from AutogeneratedClass").First();
                result.Id.IsEqualTo(5);
                result.Name.IsEqualTo("Bob");
                connection.Delete(autoGenerated);
            }
        }

        public void TestSelectingObjectWithExtraMetadata()
        {
            using (var connection = GetOpenConnection())
            {
                var autoGenerated = new AutogeneratedClass { Id = 7, Name = "Bob" };
                var id = connection.Insert(autoGenerated);
                var result = connection.Get<AutogeneratedClass>(id);
                result.Id.IsEqualTo(7);
                result.Name.IsNull();
                connection.Delete(autoGenerated);
            }
        }

        public async void TestInsertingObjectWithExtraMetadataAsync()
        {
            using (var connection = GetOpenConnection())
            {
                var autoGenerated = new AutogeneratedClass { Id = 9, Name = "Bob" };
                await connection.InsertAsync(autoGenerated);
                var result = connection.Query<AutogeneratedClass>("select * from AutogeneratedClass").First();
                result.Id.IsEqualTo(9);
                result.Name.IsEqualTo("Bob");
                connection.Delete(autoGenerated);
            }
        }

        public async void TestSelectingObjectWithExtraMetadataAsync()
        {
            using (var connection = GetOpenConnection())
            {
                var autoGenerated = new AutogeneratedClass { Id = 3, Name = "Bob" };
                var id = connection.Insert(autoGenerated);
                var result = await connection.GetAsync<AutogeneratedClass>(id);
                result.Id.IsEqualTo(3);
                result.Name.IsNull();
                connection.Delete(autoGenerated);
            }
        }
