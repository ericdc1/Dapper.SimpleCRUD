...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        /// <param name="orderByCols"></param>
        /// <param name="transaction"></param>
        /// <param name="commandTimeout"></param>
        /// <returns>Gets a list of entities with optional exact match where conditions</returns>
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, object whereConditions, string orderByCols, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            if (!idProps.Any())
            ...
            {
                sb.Append(" Order by " + orderByCols);
            }
        ...
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, string orderBy)
        {
            return connection.GetList<T>(new { }, orderBy);
        }
        ...
        {
            // Fix to the Insert failure, to check on the right type for the PK.
            var idProps = ValidateForPrimaryKey(entityToInsert);
            var keytype = ValidatePrimaryKeyType(idProps);

            var keyHasPredefinedValue = true;
            ...
            // If PK value is 0, then its IDENTITY.
            var pkVal = Convert.ToInt64(idProps.First().GetValue(entityToInsert, null));
            if ((keytype == typeof(int) || keytype == typeof(long)) && pkVal == 0)
            {
        ...
        /// <summary>
        /// Helper method to validate a given entity has a PK.
        /// </summary>
        /// <param name="obj"></param>
        private static IEnumerable<PropertyInfo> ValidateForPrimaryKey(object obj)
        {
            // Dapper restricts to only have 1 PK for some reason, so I'm keeping that restriction
            // but factored this code out of the Insert<TKey>().
            var idProps = GetIdProperties(obj).ToList();
        ...
        private static Type ValidatePrimaryKeyType(IEnumerable<PropertyInfo> idProps)
        {
            // FIX: Dapper code was checking on the wrong object to get the column type which caused the insert error.
            var baseType = idProps.First().PropertyType.FullName;
            var keytype = Type.GetType(baseType);
            if (keytype != typeof(int) && keytype != typeof(uint) &&
                keytype != typeof(long) && keytype != typeof(ulong) &&
                keytype != typeof(short) && keytype != typeof(ushort) &&
                keytype != typeof(Guid))
            {
                throw new Exception("Invalid PK type. It can only be one of these: int/uint/long/ulong/short/ushort/Guid");
            }
        ...
        private static void BuildSelect(StringBuilder sb, IEnumerable<PropertyInfo> props)
        {
            ...
            for (var i = 0; i < propertyInfos.Count(); i++)
            {
                ...
                // FIX:ND: I commented this code out as it was the cause of not mapping the value into properties tagged with the ColumnAttribute, so
                //         we shouldn't be doing " as colName".
