...
namespace Dapper
{
  ...
  public static class SimpleCRUD
  {
    ...
    public static Task<T> GetAsync<T>(this IDbConnection connection, object id)
    {
      ...
      return GetAsyncImpl<T>(connection, id, idProps, currenttype);
    }

    private static async Task<T> GetAsyncImpl<T>(this IDbConnection connection, object id, IEnumerable<PropertyInfo> idProps, Type currenttype)
    {
      var onlyKey = idProps.First();
      var name = GetTableName(currenttype);
    ...
    public static Task<TKey> InsertAsync<TKey>(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
    {
      ...
      return InsertAsyncImp<TKey>(connection, entityToInsert, keytype, transaction, commandTimeout);
    }

    private static async Task<TKey> InsertAsyncImp<TKey>(this IDbConnection connection, object entityToInsert, Type keytype, IDbTransaction transaction = null, int? commandTimeout = null)
    {
      //todo: add support for GUID
      ...
      var r = await connection.QueryAsync("select @@IDENTITY id", null, transaction, commandTimeout);
      return (TKey)r.First().id;
    }
    ...
    public static Task<IEnumerable<T>> QueryAsync<T>(this IDbConnection connection, object whereClause, IDbTransaction transaction = null, int? commandTimeout = null, System.Threading.CancellationToken? token = null)
    {
      ...
      var sb = new StringBuilder();
      var whereprops = GetAllProperties(whereClause).ToArray();
      sb.AppendFormat("Select * from {0}", name);
      ...
      if (Debugger.IsAttached)
        Trace.WriteLine(String.Format("Query: {0}", sb));

      return connection.QueryAsync<T>(new CommandDefinition(sb.ToString(), whereClause, transaction, commandTimeout, cancellationToken: token ?? default(System.Threading.CancellationToken)));
    }

    public static Task<IEnumerable<T>> QueryAsync<T>(this IDbConnection connection, IDictionary<string, object> whereClause, IDbTransaction transaction, int? commandTimeout = null, System.Threading.CancellationToken? token = null)
    {
      var currentType = typeof(T);
      var name = GetTableName(currentType);
